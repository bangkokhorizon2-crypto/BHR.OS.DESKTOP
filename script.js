...
function initHomeHeader() {
    ...
    // Auto-focus search on desktop if needed, or just ensure listener
}
...
function renderDesktopNavItems() {
    // Target the new bottom shortcuts container
    // We reuse the same logic but inject into #bottomNavShortcuts
    const container = document.getElementById('bottomNavShortcuts');
    if (!container) return;
    container.innerHTML = '';

    desktopNavOrder.forEach(key => {
        const item = navItemConfig[key];
        if (!item) return;

        const el = document.createElement('div');
        // Compact styling for bottom dock + hover-wiggle class
        el.className = `flex flex-col items-center justify-center cursor-pointer group hover:bg-gray-50 p-1.5 rounded-xl transition select-none w-14 h-14 hover-wiggle relative`;
        el.draggable = true;
        el.dataset.key = key;
        el.onclick = () => { switchTab('app'); openAppSubPage(key); };

        // Tooltip using Title attribute for simplicity in dock or just icon
        el.innerHTML = `
             <div class="text-${item.color}-500 transform group-hover:scale-110 transition duration-300">
                <i class="fas fa-${item.icon} text-xl"></i>
            </div>
            <span class="absolute -top-8 bg-gray-800 text-white text-[10px] py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none shadow-sm">
                ${item.label}
            </span>
        `;
...
function renderHistoryList(data) {
    ...
    data.forEach((item, index) => {
        ...
        container.innerHTML += `
            <div onclick="openHistoryDetail(${index})" style="animation-delay: ${index * 0.05}s" class="stagger-item vision-glass p-5 rounded-[2rem] mb-3 md:mb-0 glass-card-hover cursor-pointer relative overflow-visible group h-full">
...
function renderLicenseList() {
    ...
    container.innerHTML = currentLicenseList.map((item, index) => `
        <div onclick="openLicenseDetail(${index})" style="animation-delay: ${index * 0.05}s" class="stagger-item vision-glass p-5 rounded-[2rem] mb-3 md:mb-0 glass-card-hover cursor-pointer relative overflow-visible group h-full">
...
function renderOwnerList() {
    ...
    container.innerHTML = currentOwnerList.map((item, index) => `
        <div onclick="openOwnerDetail(${index})" style="animation-delay: ${index * 0.05}s" class="stagger-item vision-glass p-5 rounded-[2rem] mb-3 md:mb-0 glass-card-hover cursor-pointer relative overflow-visible group h-full">
            <div class="flex justify-between items-center relative z-10">
                <div class="flex flex-col flex-1 overflow-visible">
                    <div class="font-bold text-ios-primary text-xl">${item.room}</div>
